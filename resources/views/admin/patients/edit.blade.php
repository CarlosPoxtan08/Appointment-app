<x-admin-layout title="Editar Paciente | MediMatch" :breadcrumbs="[
        [
            'name' => 'Dashboard',
    ...
    {{--<div class="mb-4 border-b border-gray-200">--}}
            ...
            <li class="me-2" role="presentation">
                <button class="inline-flex items-center gap-2 p-4 border-b-2 rounded-t-lg" id="history-tab"
            ...
            <li role="presentation">
                <button class="inline-flex items-center gap-2 p-4 border-b-2 rounded-t-lg" id="emergency-tab"
                    data-tabs-target="#emergency" type="button" role="tab" aria-controls="emergency"
                    aria-selected="false">
    ...
    <form id="edit-patient-form" action="{{ route('admin.patients.update', $patient) }}" method="POST">
        ...
        <div id="patientTabContent">
            ...
            <div class="hidden p-6 bg-white border border-gray-200 rounded-lg shadow-sm" id="personal" role="tabpanel"
                aria-labelledby="personal-tab">
                {{-- Aviso de edición de usuario --}}
                <x-wire-alert class="mb-6" color="info" title="Edición de cuenta de usuario">
                    <strong>La información de acceso</strong> (Nombre, Email y Contraseña) debe gestionarse desde la
                    cuenta del usuario asociada.
                    <x-slot name="action">
            ...
            <div class="hidden p-6 bg-white border border-gray-200 rounded-lg shadow-sm" id="history" role="tabpanel"
                ...
                <div class="grid lg:grid-cols-2 gap-4">
                    ...
                    <x-wire-textarea label="Condiciones crónicas" name="chronic_conditions"
                        placeholder="Condiciones crónicas (opcional)" :value="old('chronic_conditions', $patient->chronic_conditions)">
                    </x-wire-textarea>

                    <x-wire-textarea label="Historial quirúrgico" name="surgical_history"
                        placeholder="Cirugías previas (opcional)" class="lg:col-span-2" :value="old('surgical_history', $patient->surgical_history)">
                    </x-wire-textarea>
                </div>
            </div>

            {{-- Pestaña 3: Información General --}}
            <div class="hidden p-6 bg-white border border-gray-200 rounded-lg shadow-sm" id="general" role="tabpanel"
                ...
                <div class="grid lg:grid-cols-2 gap-4">
                    ...
                    <x-wire-textarea label="Observaciones" name="observations"
                        placeholder="Observaciones adicionales (opcional)" class="lg:col-span-2"
            ...
            <div class="hidden p-6 bg-white border border-gray-200 rounded-lg shadow-sm" id="emergency" role="tabpanel"
                aria-labelledby="emergency-tab">
                <div class="grid lg:grid-cols-2 gap-4">
                    <x-wire-input label="Contacto" name="emergency_contact_name" placeholder="Nombre del contacto"
                        value="{{ old('emergency_contact_name', $patient->emergency_contact_name) }}">
                    </x-wire-input>

                    <x-wire-phone label="Teléfono" name="emergency_contact_phone" placeholder="Teléfono del contacto"
                        value="{{ old('emergency_contact_phone', $patient->emergency_contact_phone) }}" inputmode="tel">
                    </x-wire-phone>

                    <x-wire-input label="Parentesco" name="emergency_relationship"
